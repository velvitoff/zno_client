...
class _PastSessionsListState extends State<PrevSessionsList> {
  ...
  void initState() {
    super.initState();
    dataList = locator.get<StorageService>()
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Center(
        child: FutureBuilder(
          future: dataList,
          builder: (BuildContext context, AsyncSnapshot<List<PreviousSessionData>> snapshot) {
            if (snapshot.hasData) {
              ...
              return Text(
                'Помилка завантаження попередніх спроб',
                style: TextStyle(
                    color: const Color(0xFF5F5F5F),
                    fontSize: 20.sp,
                    fontWeight: FontWeight.w400
