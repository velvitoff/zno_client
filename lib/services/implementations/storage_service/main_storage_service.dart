...
class MainStorageService with StorageService {
  ...
  MainStorageService._create(
    ExternalStorageService external,
  ...
  static Future<MainStorageService> create({
    required ExternalStorageService externalStorageService,
  }) async {
  ...
  Future<List<String>> listSessions(String folderName) async {
    ...
    catch(e) { //fallback for listing sessions in case of network exception
      return await localStorage.listSessions(folderName);
    }
  }

  @override
  Future<String> getSession(String folderName, String fileName) async { //throws
    try {
      //return local file if it's recent enough
      var lastModified = await localStorage.getSessionDate(folderName, fileName);
      ...
      try {
        final String session = await externalStorage.getSession(folderName, fileName);
        localStorage.saveSession(folderName, fileName, session); //not awaited
        await downloadImages(folderName, fileName);
        return session;
      }
      catch(e) {//if the download attempt fails, try using local data without a recency restriction
        return localStorage.getSession(folderName, fileName);
      }
    }
  ...
  Future<void> downloadImages(String subjectName, String sessionName) async {
    ...
    var imageMap = await externalStorage.downloadAllImages(subjectName, sessionName);
