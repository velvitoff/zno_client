...
class ZnoCross extends CustomPainter {
  final Color color;
  const ZnoCross({required this.color});

  @override
  void paint(Canvas canvas, Size size) {
    Paint paint0Stroke = Paint()
      ..style = PaintingStyle.stroke
      ..strokeWidth = size.width * 0.08510638;
    paint0Stroke.color = color;
    canvas.drawRRect(
        RRect.fromRectAndCorners(
            Rect.fromLTWH(size.width * 0.04295830, size.height * 0.04440304,
                size.width * 0.9136191, size.height * 0.9121174),
            bottomRight: Radius.circular(size.width * 0.1063830),
            bottomLeft: Radius.circular(size.width * 0.1063830),
            topLeft: Radius.circular(size.width * 0.1063830),
            topRight: Radius.circular(size.width * 0.1063830)),
        paint0Stroke);
